...
local act_cli = rclbind.new_action_client(node, Fibonacci, 'fibonacci', nil, 
...
local function async_send (goal)
  if not rclbind.is_instance(goal, act_cli:get_interface 'Goal') then error('Not a goal message') end
  local uuid, uuid_str = rclbind.get_uuid()
  local send_goal = act_cli:get_interface('SendGoal')
  local req = send_goal.Request()
  if not req.goal_id.uuid(uuid) then error('Unable to set UUID') end
  req.goal = goal
...
local function cancel_goal (uuid)
  local req = Fibonacci.CancelGoal.Request()
...
while rclbind.context_ok() do
  ...
  if data['status'] then
    ...
    local status_list = msg.status_list
  end
