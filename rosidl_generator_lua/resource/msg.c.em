...
@{
from rosidl_generator_lua.generate_lua_impl import NUMERIC_LUA_TYPES
...
@#  constructor
static int @(msg_prefix)_lnew (lua_State* L) {
  idl_lua_msg_t* ptr = lua_newuserdata(L, sizeof(idl_lua_msg_t));
  ptr->value = IDL_LUA_OBJECT;

  @(msg_prefix)* msg = @(msg_prefix)_create();
  if (NULL == msg) {
    luaL_error(L, "failed to create message");
  }
  @(msg_prefix)__init(msg);
  ptr->obj = msg;

  luaL_getmetatable(L, "@(msg_prefix)_mt");
...
@#  destructor
static int @(msg_prefix)_lgc (lua_State* L) {
  idl_lua_msg_t* ptr = lua_touserdata(L, 1);
  if (IDL_LUA_OBJECT == ptr->value) {
    @(msg_prefix)__fini(ptr->obj);
    @(msg_prefix)__destroy(ptr->obj);
  }
...
@#  check equality
static int @(msg_prefix)_leq (lua_State* L) {
  // check equality
  ...
  if (p1->value < IDL_LUA_SEQ && p2->value < IDL_LUA_SEQ) {
    // objects
    lua_pushboolean(@(msg_prefix)__are_equal(p1->obj, p2->obj));
  } else if (p1->value >= IDL_LUA_SEQ && p2->value >= IDL_LUA_SEQ) {
    // lists
    @(msg_prefix)__Sequence s1, s2;
    if (p1->value == IDL_LUA_SEQ) {
      s1 = *(@(msg_prefix)__Sequence*) p1->obj;
    } else {
      s1.data = p1->obj;
      s1.size = s1.capacity = p1->value;
    }
    if (p2->value == IDL_LUA_SEQ) {
      s2 = *(@(msg_prefix)__Sequence*) p2->obj;
    } else {
      s2.data = p2->obj;
      s2.size = s2.capacity = p2->value;
    }
    lua_pushboolean(@(msg_prefix)__Sequence__are_equal(&s1, &s2));
  } else {
    lua_pushboolean(L, false);
  }
...
@# copy value
static int @(msg_prefix)_lcopy (lua_State* L) {
  // check equality
  lua_getmetatable(L, 1);
  lua_getmetatable(L, 2);
  if (lua_isnil(L, -1) || !lua_rawequal(L, -1, -2)) {
  ...
  if (dst->value < IDL_LUA_SEQ && src->value < IDL_LUA_SEQ) {
    // objects
    done = @(msg_prefix)__copy(src->obj, dst->obj);
  } else if (dst->value == IDL_LUA_SEQ) {
    // list
    if (src->value == IDL_LUA_SEQ) {
      done = @(msg_prefix)__Sequence__copy(src->obj, dst->obj);
    } else if (src->value > 0) {
      @(msg_prefix)__Sequence tmp;
      tmp.data = src->obj;
      tmp.size = tmp.capacity = (size_t) src->value;
      done = @(msg_prefix)__Sequence__copy(&tmp, dst->obj);
    }
  } else if (dst->value > 0) {
    // array
    @(msg_prefix)__Sequence *a = dst->obj, *b = NULL;
    if (src->value > 0 && dst->value == src->value) {
      b = src->obj;
    } else if (src->value == IDL_LUA_SEQ) {
      @(msg_prefix)__Sequence* seq = src->obj;
...
@# get length
static int @(msg_prefix)_llen (lua_State* L)
{
  idl_lua_msg_t* ptr = lua_touserdata(L, 1);
  if (ptr->value > 0) {
    lua_pushnumber(L, ptr->value);
  } else if (ptr->value == IDL_LUA_SEQ) {
    @(msg_prefix)__Sequence* seq = ptr->obj;
...
@# to string
static int @(msg_prefix)_lstr (lua_State* L)
{
  idl_lua_msg_t* ptr = lua_touserdata(L, 1);
  if (ptr->value > 0) {
    lua_pushfstring(L, "@(msg_type) array of size %d", ptr->value);
  } else if (ptr->value == IDL_LUA_SEQ) {
    @(msg_prefix)__Sequence* seq = ptr->obj;
    lua_pushfstring(L, "@(msg_type) sequence of size %d", seq->size);
  } else { 
    lua_pushfstring(L, "@(msg_type)");
  } 
...
@# resize sequence
static int @(msg_prefix)_lresize (lua_State* L) 
{ 
  idl_lua_msg_t* ptr = luaL_checkudata(L, 1, "@(msg_prefix)_mt");
  ...
  if (seq->capacity == 0) { 
    done = @(msg_prefix)__Sequence__init(seq, len);
  } else if (seq->capacity >= (size_t) len) { 
    seq->size = (size_t) len;
  } else {
    @(msg_prefix)__Sequence newseq;
    if (@(msg_prefix)__Sequence__init(&newseq, len) &&
       @(msg_prefix)__Sequence__copy(seq, &newseq))
    { 
      @(msg_prefix)__Sequence tmp = *seq;
      *seq = newseq; 
      @(msg_prefix)__Sequence__fini(&tmp);
    } else { 
      done = false;
    }
  }
...
@[  end if]@
@{
setter_ = '_'.join((msg_prefix, member.name, 'set'))
}@
...
nested_type = '__'.join(type_.namespaced_name())
  ...
  lua_getmetatable(L, "@(msg_prefix)_mt");
...
@[  end if]@
@{
getter_ = '_'.join(msg_prefix, member.name, 'get')
}@
...
static int @(msg_prefix)_lindex (lua_State* L) {
  lua_getmetatable(L, 1);
...
static int @(msg_prefix)_lnewindex (lua_State* L) {
  lua_getmetatable(L, 1);
  idl_lua_msg_t* msg = lua_touserdata(L, 1);
